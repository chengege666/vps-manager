...
port_manage() {
    ...
    case "$port_choice" in
        1)
            ...
            if command -v ufw >/dev/null 2>&1; then ufw allow "$add_port"; echo "端口 $add_port 已允许"; else echo "请手动添加防火墙规则"; fi
            ;;
        2)
            read -rp "请输入端口号: " del_port
            if command -v ufw >/dev/null 2>&1; then ufw delete allow "$del_port"; echo "端口 $del_port 已删除"; else echo "请手动删除防火墙规则"; fi
            ;;
...
docker_manage() {
    if ! command -v docker >/dev/null 2>&1; then
        ...
        if [[ "$install_choice" =~ ^[Yy]$ ]]; then
            ...
            if command -v curl >/dev/null 2>&1; then curl -fsSL https://get.docker.com -o get-docker.sh; elif command -v wget >/dev/null 2>&1; then wget -O get-docker.sh https://get.docker.com; else echo "错误: 未找到 curl 或 wget。"; return; fi
            if [ ! -f "get-docker.sh" ]; then echo "错误: Docker 安装脚本下载失败。"; return; fi
            sudo sh get-docker.sh --mirror Aliyun; rm get-docker.sh
    ...
    case "$ssh_choice" in
        1)
            ...
            if [[ "$new_port" -ge 1024 && "$new_port" -le 65535 ]]; then
                ...
                if command -v ufw >/dev/null 2>&1; then ufw allow "$new_port"; echo "UFW: 已允许端口 $new_port"; elif command -v iptables >/dev/null 2>&1; then iptables -I INPUT -p tcp --dport "$new_port" -j ACCEPT; iptables-save > /etc/iptables/rules.v4 2>/dev/null || true; echo "iptables: 已允许端口 $new_port"; else echo "未检测到防火墙管理工具，请手动放行端口"; fi
                systemctl restart sshd; echo "SSH 端口已修改为 $new_port"
            else echo "端口号不合法"; fi
            ;;
        2)
            read -rp "请输入用户名: " user_name
            if id "$user_name" >/dev/null 2>&1; then passwd "$user_name"; else echo "用户不存在"; fi
            ;;
        3) return ;;
        *) echo "无效选项" ;;
    esac
...
while true; do
    ...
    case "$choice" in
        1) sys_info; read -rp "按 Enter 键返回主菜单..." ;;
        2) sys_update; read -rp "按 Enter 键返回主菜单..." ;;
        3) sys_clean; read -rp "按 Enter 键返回主菜单..." ;;
        4) port_manage; read -rp "按 Enter 键返回主菜单..." ;;
        5) docker_manage; read -rp "按 Enter 键返回主菜单..." ;;
        6) ssh_manage; read -rp "按 Enter 键返回主菜单..." ;;
        7) clear; exit 0 ;;
        *) 
           echo "无效选项，请重新输入。"
           sleep 2 # 暂停2秒给用户看提示
